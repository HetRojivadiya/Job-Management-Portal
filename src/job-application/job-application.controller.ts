...
export class JobApplicationController {
  ...
  async getAppliedJobs(@Param('userId') userId: string) : Promise<ApiResponse<AppliedJob[]>> {
    try {
      const appliedJobs = await this.jobApplicationService.getAppliedJobs(userId);
      return {
  ...
  async deleteAppliedJob(
    ...
    try {
      await this.jobApplicationService.deleteAppliedJob(req.user.id,applicationId);
      return {
        statusCode: HttpStatus.OK,
  ...
  async applicants(@Body() body: { jobId: string }) : Promise<ApiResponse<UserResponse[]>>{
    try {
      const data = await this.jobApplicationService.getJobApplicantsIds(body.jobId);
      if (!data) {
